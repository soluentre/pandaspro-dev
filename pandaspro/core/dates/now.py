...
By default, the function will seperate  ... 
    ...
    if override == False:
        format = format_mapping[format] if format in format_mapping.keys() else ''.join(filter(lambda x: x.isalpha() or x == '%', format))
...
def currentdate(display='textDMY'):
    ...
    if display == 'textDMY':
        return datetime.now().strftime("%d") + " " + month_mapping[int(datetime.now().strftime("%Y-%m").split('-')[1])] + " " + datetime.now().strftime("%Y-%m").split('-')[0]
